...
router.post("/cart", async (req: Request, res: Response): Promise<any> => {
  try {
...
router.patch("/cart", async (req: Request, res: Response): Promise<any> => {
  const response = await service.EditCart(req.body, repo);
  return res.status(200).json(response);
});

router.delete("/cart", async (req: Request, res: Response): Promise<any> => {
